...
async function main() {
    ...
    function onReadComplete(device, pipeline) {
        ...
        console.log(new Float32Array(emissions));
        // Set up buffers
        ...
        device.queue.writeBuffer(mColorsBuffer, 0, new Float32Array(colors));
        const mEmissionsBuffer = device.createBuffer({
            size: Float32Array.BYTES_PER_ELEMENT * emissions.length,
            usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
        });
        device.queue.writeBuffer(mEmissionsBuffer, 0, new Float32Array(emissions));
        const mIndicesBuffer = device.createBuffer({
        ...
        entries: [
            ...
            {
                binding: 3,
                resource: { buffer: mColorsBuffer },
            },
            {
                binding: 4,
                resource: { buffer: mEmissionsBuffer },
            },
            {
                binding: 5,
